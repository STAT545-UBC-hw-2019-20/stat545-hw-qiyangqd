...
lifeExp_comp_wider <- lifeExp_comp %>%  # Put data in wider format
                      ...
                      names_from = country,
                      values_from = lifeExp) %>% 
                      rename(lifeExp_CAR = "Central African Republic", lifeExp_Japan = "Japan") # Rename to make the tibble clear
...
lifeGDP <- gapminder %>%  # Create new dataset: lifeGDP
  select(year, country, lifeExp, gdpPercap) %>%  # Select columns needed by lifeExp_comp
  filter(country == "Central African Republic" | country == "Japan") # Choose two countries we are interested in
...
```
This time I still chose Central African Republic (CAR) and Japan. This tibble shows the life expectancy and GDP per capita of these two countries across years. We can see tremendous gaps between them: e.g. In 1997, Japan's GDP per capita was almost 40 times higher than CAR! Japanese also lived twice as long as people in CAR, indicated by the life expectancy values.
...
lifeGDP_wider %>% 
  rename("lifeExp_Central African Republic" = "lifeExp_CAR", "gdpPercap_Central African Republic" = "gdpPercap_CAR") %>% 
                                                                              # Give their original names back to columns
    pivot_longer(cols      = c(-year), 
