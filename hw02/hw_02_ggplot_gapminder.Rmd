...
gapminder %>% 
  ...
  nrow()
```
So there are 142 different values (countries). Since the space is limited, let's just see the first and last few rows of it (arranged alphabetically).   
...
- According to Google (one of the pages can be found  [here](http://www.physics.csbsju.edu/stats/descriptive2.html)), typical values can be represented by mean or median.   
- **Mean** of pop:
```{r, warning = FALSE}
gapminder %>% 
  select(pop) %>% 
